...
def main():
    ...
    df['count'].plot(kind='bar', color='springgreen', ax=ax, width=width, position=1)

    ax.set_ylabel('Eggnog annotations count', fontsize=16)
    ax.set_xlabel('Sequence', fontsize=16)
    #plt.title('Histogram of ', fontsize=12)
    trans_patch = mpatches.Patch(color='springgreen', label='Eggnog annotations')
    plt.legend(handles=[trans_patch], frameon=False, fontsize=16)
    plt.xticks(fontsize=16)
    plt.yticks(fontsize=16)
    plt.tight_layout()
    plt.savefig('eggnog.png')

...
def main():
    ...
    df['count'].plot(kind='bar', color='deeppink', ax=ax, width=width, position=1)

    ax.set_ylabel('Operon Count', fontsize=16)
    ax.set_xlabel('Sequence', fontsize=16)
    #plt.title('Histogram of ', fontsize=12)
    trans_patch = mpatches.Patch(color='deeppink', label='Operon')
    plt.legend(handles=[trans_patch], frameon=False, fontsize=16)
    plt.xticks(fontsize=16)
    plt.yticks(fontsize=16)
    plt.tight_layout()
    plt.savefig('operon.png')
